    ...
    <div style={{ fontFamily: "'DM Sans', 'Inter', sans-serif", background: "#fff", minHeight: "100vh" }}>
      ...
      <style>{`
        @media (max-width: 768px) {
          ...
          .kt-card-full { min-height: 240px !important; border-radius: 12px !important; }
          .kt-card-full-text { padding: 1.5rem !important; max-width: 100% !important; }
          .kt-card-full-title { font-size: 18px !important; }
          .kt-card-left { min-height: 240px !important; border-radius: 12px !important; }
          .kt-card-plain { border-radius: 12px !important; }
        }
        ...
        .kt-card-plain:hover { border-color: rgba(0,0,0,0.15) !important; }
      ...
      {/* ARTICLE FEED */}
      <div className="kt-feed" style={{ maxWidth: "1100px", margin: "0 auto", padding: "0 40px 60px", display: "flex", flexDirection: "column", gap: "16px" }}>
        ...
        {articles.length === 0 ? (
          ...
          rows.map((row) => {
                ...
                <Link key={a.id} href={`/hoc-vien/nen-tang-kien-thuc/${a.slug}`} style={{ textDecoration: "none" }}>
                  <div className="kt-card-full" style={{
                    ...
                    <div style={{
                      ...
                      background: "linear-gradient(to right, rgba(5,15,30,0.95) 0%, rgba(5,15,30,0.95) 45%, rgba(5,15,30,0.4) 65%, rgba(5,15,30,0.0) 100%)",
                    }} />
                    <div className="kt-card-full-text" style={{ position: "relative", padding: "2.5rem", maxWidth: "55%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "center" }}>
                      {a.knowledge_categories?.name && (
                        <span style={{
                          display: "inline-block", fontSize: "11px", fontWeight: 600,
                          color: "#00C389", textTransform: "uppercase", letterSpacing: "0.08em",
                          marginBottom: "10px",
                        }}>
                          {a.knowledge_categories.name}
                        </span>
                      )}
                      <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)", marginBottom: "14px", letterSpacing: "0.02em" }}>
                        Thời gian đọc · {a.read_time || 5} phút
                      </p>
                      <h2 className="kt-card-full-title" style={{
                        fontSize: "26px", fontWeight: 700, color: "#fff",
                        lineHeight: 1.3, marginBottom: "14px", letterSpacing: "-0.01em",
                      }}>{a.title}</h2>
                      {a.excerpt && (
                        <p style={{
            ...
            return (
              <div key={left.id} className="kt-split-row" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                ...
                <Link href={`/hoc-vien/nen-tang-kien-thuc/${left.slug}`} style={{ textDecoration: "none" }}>
                  <div className="kt-card-left" style={{
                    ...
                    background: left.thumbnail_url
                      ...
                      : "linear-gradient(135deg, #0D1F38 0%, #0A2540 100%)",
                    minHeight: "320px", position: "relative", display: "flex", alignItems: "flex-end",
                    transition: "opacity 0.15s",
                  }}>
                    <div style={{
                      position: "absolute", inset: 0,
                      background: "linear-gradient(to top, rgba(5,15,30,0.97) 0%, rgba(5,15,30,0.5) 55%, transparent 100%)",
                    }} />
                    <div style={{ position: "relative", padding: "1.75rem" }}>
                      {left.knowledge_categories?.name && (
                        <span style={{
                          display: "inline-block", fontSize: "11px", fontWeight: 600,
                          color: "#00C389", textTransform: "uppercase", letterSpacing: "0.08em",
                          marginBottom: "8px",
                      ...
                      <h3 style={{ fontSize: "20px", fontWeight: 700, color: "#fff", lineHeight: 1.35, marginBottom: "10px" }}>
                        {left.title}
                      </h3>
                      {left.excerpt && (
                        <p style={{
                ...
                {right ? (
                    ...
                    <div className="kt-card-plain" style={{
                      ...
                      {right.thumbnail_url && (
                        <div style={{
                          ...
                          background: `url(${right.thumbnail_url}) center/cover no-repeat`,
                        }} />
                      )}
                      {!right.thumbnail_url && (
                        <div style={{
                          width: "100%", height: "180px", flexShrink: 0,
                          background: "linear-gradient(135deg, #0D1F38 0%, #1a3a5c 100%)",
                        }} />
                      )}
                      {/* Text */}
                      <div style={{ padding: "1.5rem", display: "flex", flexDirection: "column", flex: 1, justifyContent: "space-between" }}>
                        <div>
                          {right.knowledge_categories?.name && (
                            <span style={{
                              display: "inline-block", fontSize: "11px", fontWeight: 600,
                              color: "#00C389", textTransform: "uppercase", letterSpacing: "0.08em",
                              marginBottom: "8px",
                          ...
                          <h3 style={{ fontSize: "17px", fontWeight: 700, color: "#0A1628", lineHeight: 1.4, marginBottom: "10px" }}>
                            {right.title}
                          </h3>
                          {right.excerpt && (
                            <p style={{
                              fontSize: "13px", color: "#64748b", lineHeight: 1.65, marginBottom: "16px",
                              display: "-webkit-box", WebkitLineClamp: 3,
                              WebkitBoxOrient: "vertical", overflow: "hidden",
                  ...
                  <div style={{ borderRadius: "16px", background: "#f8fafc", border: "1px solid #f1f5f9" }} />
                )}
              </div>
            )
          })
