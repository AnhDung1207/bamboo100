...
const GROUP_ICONS: Record<string, { icon: string; color: string }> = {
  "hot-news":                { icon: "ti-flame",            color: "#dc2626" },
  "kim-loai":                { icon: "ti-medal",            color: "#EF9F27" },
  "nong-san":                { icon: "ti-plant-2",          color: "#639922" },
  "nguyen-lieu-cong-nghiep": { icon: "ti-building-factory", color: "#BA7517" },
  "nang-luong":              { icon: "ti-bolt",             color: "#E24B4A" },
  "phan-tich-ky-thuat":      { icon: "ti-chart-candle",     color: "#6D28D9" },
  "bao-cao-kinh-te":         { icon: "ti-file-text",        color: "#1D4ED8" },
}
...
export async function generateMetadata({
  ...
  const { data } = await supabase
    .from("articles")
    .select(`
      title, excerpt, thumbnail_url, published_at, updated_at,
      categories ( name ),
      profiles ( full_name )
    `)
    .eq("slug", slug)
  ...
  if (!data) return { title: "Bài viết | BAMBOO100" }

  const title = data.title
  const description = data.excerpt ?? "Phân tích chuyên sâu về thị trường hàng hóa từ BAMBOO100."
  const image = data.thumbnail_url ?? `${BASE_URL}/og-default.jpg`
  const url = `${BASE_URL}/phan-tich/${slug}`
  const authorName = (data.profiles as any)?.full_name ?? "BAMBOO100"
  const categoryName = (data.categories as any)?.name
...
export default async function ArticleDetailPage({
  ...
  const { data: article } = await supabase
    ...
    .select(`
      ...
      key_takeaways,
      is_premium, is_hot, view_count, read_time, published_at, updated_at,
  ...
  const groupSlug = a.product_groups?.slug ?? ""
  ...
  const { data: related } = await supabase
    .from("articles")
    .select("id, title, slug, thumbnail_url, published_at, categories(name, slug)")
    .eq("status", "published")
    .neq("slug", slug)
  ...
  const jsonLd = {
    ...
    author: { "@type": "Person", name: a.profiles?.full_name ?? "BAMBOO100" },
    publisher: {
      "@type": "Organization", name: "BAMBOO100",
      logo: { "@type": "ImageObject", url: `${BASE_URL}/o.png` },
    },
    mainEntityOfPage: { "@type": "WebPage", "@id": `${BASE_URL}/phan-tich/${a.slug}` },
  }
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', 'Inter', sans-serif", minHeight: "100vh", background: "#fff" }}>
      ...
      <div className="breadcrumb" style={{ background: "#f7f8fa", borderBottom: "1px solid #e8ecef", padding: "10px 40px", fontSize: "12px", color: "#64748b", display: "flex", alignItems: "center", flexWrap: "wrap", gap: "6px" }}>
        ...
        <span>›</span>
        <span style={{ color: catColor, fontWeight: 500 }}>{a.categories?.name ?? "Hàng hóa"}</span>
      ...
      <div className="article-wrapper" style={{ maxWidth: "1440px", margin: "0 auto", padding: "32px 40px" }}>
        <div className="article-grid" style={{ display: "grid", gridTemplateColumns: "22% minmax(0, 1fr) 18%", gap: "32px", alignItems: "start" }}>
          ...
          <aside className="sidebar-left" style={{ position: "sticky", top: "80px", alignSelf: "start" }}>
            <div className="sidebar-left-prices"><PriceWidget /></div>
          ...
          <aside className="sidebar-right" style={{ position: "sticky", top: "80px", alignSelf: "start" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
              {(productGroups ?? []).map((grp: any) => {
                const gi = GROUP_ICONS[grp.slug]
                const isActive = groupSlug === grp.slug
                return (
                  <Link key={grp.id} href={`/phan-tich?group=${grp.slug}`} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "6px", padding: "10px 8px", borderRadius: "8px", border: isActive ? `1px solid ${gi?.color ?? "#00C389"}` : "1px solid #e2e8f0", background: isActive ? `${gi?.color ?? "#00C389"}12` : "#fff", textDecoration: "none", textAlign: "center" }}>
                    <i className={`ti ${gi?.icon ?? "ti-chart-bar"}`} style={{ fontSize: "18px", color: gi?.color ?? "#00C389" }} aria-hidden="true" />
                    <span style={{ fontSize: "11px", lineHeight: 1.3, color: isActive ? (gi?.color ?? "#00C389") : "#334155", fontWeight: isActive ? 600 : 400 }}>{grp.name}</span>
                  </Link>
                )
              })}
            </div>
