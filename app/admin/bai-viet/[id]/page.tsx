...
export default function SuaBaiViet() {
  ...
  const [form, setForm] = useState({
    ...
    category_id: "", group_id: "", read_time: "",
    is_premium: false, is_hot: false,
    status: "draft" as "draft" | "published",
    published_at: "", key_takeaways: "",
  })
    ...
    Promise.all([
      ...
      if (article) {
        setForm({
          ...
          thumbnail_url: article.thumbnail_url || "", category_id: article.category_id || "",
          group_id: article.group_id || "", read_time: article.read_time?.toString() || "",
          is_premium: article.is_premium || false, is_hot: article.is_hot || false,
          status: article.status || "draft",
          published_at: article.published_at
            ? new Date(new Date(article.published_at).getTime() + 7 * 60 * 60 * 1000).toISOString().slice(0, 16) : "",
  ...
  useEffect(() => {
    if (form.group_id) setFilteredCategories(categories.filter((c) => c.group_id === form.group_id))
    else setFilteredCategories([])
  }, [form.group_id, categories])

  useEffect(() => {
    if (!fetching && form.content && editorRef.current) editorRef.current.innerHTML = form.content
  }, [fetching])
  ...
  const handleEditorImageUpload = async (file: File) => {
    ...
    const { error } = await supabase.storage.from("thumbnails").upload(fileName, compressed, { cacheControl: "3600", upsert: false, contentType: "image/jpeg" })
    if (error) return
  ...
  const inputStyle = { width: "100%", padding: "7px 10px", borderRadius: "7px", border: "0.5px solid #e2e8f0", fontSize: "13px", color: "#0A1628", background: "#fff", outline: "none", boxSizing: "border-box" as const }
  const labelStyle = { fontSize: "11px", fontWeight: 500 as const, color: "#64748b", display: "block" as const, marginBottom: "5px" }
      ...
      <div style={cardStyle}>
        ...
        {thumbnailPreview ? (
          <div style={{ position: "relative" }}>
            ...
            <button onClick={() => { setThumbnailPreview(""); setForm((p) => ({ ...p, thumbnail_url: "" })) }}
          ...
          <div onClick={() => fileInputRef.current?.click()} style={{ border: "1.5px dashed #e2e8f0", borderRadius: "8px", padding: "24px", textAlign: "center", cursor: "pointer", background: "#f8fafc" }}>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          <div>
            ...
            <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as "draft" | "published" })} style={inputStyle}>
              <option value="draft">Nháp</option>
              <option value="published">Đã đăng</option>
            </select>
          </div>
          <div>
            <label style={labelStyle}>Thời gian đăng</label>
    ...
    <div style={{ fontFamily: "'DM Sans', 'Inter', sans-serif", minHeight: "100vh", background: "#f8fafc" }}>
      <style>{`
        @media (max-width: 767px) {
          ...
          .editor-sidebar-desktop { display: none !important; }
          .editor-topbar { padding: 10px 14px !important; }
          .editor-topbar-title { display: none !important; }
          .editor-content { padding: 14px !important; }
          .editor-title-input { font-size: 18px !important; }
      ...
      <div className="editor-topbar" style={{ background: "#fff", borderBottom: "0.5px solid #e2e8f0", padding: "14px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 40 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <span className="editor-topbar-title" style={{ color: "#e2e8f0" }}>|</span>
          <h1 className="editor-topbar-title" style={{ fontSize: "15px", fontWeight: 600, color: "#0A1628" }}>Sửa bài viết</h1>
        </div>
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          ...
          <button onClick={() => handleSave("published")} disabled={loading} style={{ padding: "7px 16px", borderRadius: "7px", background: "#00C389", border: "none", color: "#fff", fontSize: "13px", fontWeight: 500, cursor: loading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: "5px", fontFamily: "inherit" }}>
        ...
        <div className="editor-layout" style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: "20px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <div style={{ background: "#fff", borderRadius: "12px", border: "0.5px solid #e2e8f0", padding: "16px 24px" }}>
              ...
              <textarea placeholder="Mô tả ngắn về bài viết..." value={form.excerpt} onChange={(e) => setForm({ ...form, excerpt: e.target.value })} rows={3}
            ...
            <div style={{ background: "#fff", borderRadius: "12px", border: "0.5px solid #e2e8f0", overflow: "hidden" }}>
              <div style={{ padding: "12px 24px", borderBottom: "0.5px solid #e2e8f0", background: "#f8fafc", display: "flex", gap: "4px", flexWrap: "wrap" }}>
                <input ref={imageInputRef} type="file" accept="image/*" style={{ display: "none" }}
                  onChange={(e) => { const f = e.target.files?.[0]; if (f) handleEditorImageUpload(f); e.target.value = "" }} />
                {[
                  ...
                  <button key={btn.cmd} onMouseDown={(e) => {
                    e.preventDefault()
                    if (btn.cmd === "h2") { const sel = window.getSelection(); const block = sel?.anchorNode?.parentElement?.closest("h2"); document.execCommand("formatBlock", false, block ? "p" : "h2") }
                    else if (btn.cmd === "h3") { const sel = window.getSelection(); const block = sel?.anchorNode?.parentElement?.closest("h3"); document.execCommand("formatBlock", false, block ? "p" : "h3") }
                    else if (btn.cmd === "formatBlock") { const sel = window.getSelection(); const block = sel?.anchorNode?.parentElement?.closest("blockquote"); document.execCommand("formatBlock", false, block ? "p" : "blockquote") }
                    else if (btn.cmd === "createLink") { const url = prompt("Nhập URL:"); if (url) document.execCommand("createLink", false, url) }
                    else document.execCommand(btn.cmd, false)
                    ...
                    <i className={`ti ${btn.icon}`} style={{ fontSize: "14px" }} />
                  </button>
                ))}
                <button onMouseDown={(e) => { e.preventDefault(); imageInputRef.current?.click() }}
                  style={{ width: "30px", height: "30px", borderRadius: "5px", background: "transparent", border: "0.5px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#64748b" }}>
                  <i className="ti ti-photo" style={{ fontSize: "14px" }} />
                </button>
              </div>
          ...
          <div className="editor-sidebar-desktop" style={{ display: "flex", flexDirection: "column", gap: "14px", position: "sticky", top: "80px", alignSelf: "start" }}>
            <SidebarContent />
          </div>
        </div>
      </div>
      ...
      {sidebarOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
          ...
          <div style={{ position: "relative", background: "#f8fafc", borderRadius: "16px 16px 0 0", maxHeight: "85vh", overflowY: "auto", animation: "slideUp 0.25s ease" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", background: "#fff", borderRadius: "16px 16px 0 0", borderBottom: "0.5px solid #e2e8f0", position: "sticky", top: 0 }}>
              <span style={{ fontSize: "15px", fontWeight: 600, color: "#0A1628" }}>Cài đặt bài viết</span>
              <button onClick={() => setSidebarOpen(false)} style={{ background: "transparent", border: "none", fontSize: "20px", color: "#64748b", cursor: "pointer" }}>✕</button>
            </div>
