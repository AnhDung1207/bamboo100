...
export default async function AdminLeadsPage({
  ...
  const SOURCE_LABELS: Record<string, string> = {
    homepage: "Trang chủ", contact: "Liên hệ",
    article: "Bài viết",   ebook: "Ebook", course: "Khóa học",
  }

  const csvContent = ["Họ tên,Email,SĐT,Nguồn,Trạng thái,Ngày tạo"]
    ...
    <div style={{ fontFamily: "'DM Sans', 'Inter', sans-serif" }}>
      <style>{`
        @media (max-width: 767px) {
          /* Topbar */
          .ld-topbar  { padding: 12px 16px !important; }
          .ld-content { padding: 16px !important; }
          .ld-csv-text { display: none !important; }

          /* Stats 2x2 */
          .ld-stats   { grid-template-columns: repeat(2, 1fr) !important; gap: 10px !important; }

          /* Filters scroll ngang */
          .ld-filters { flex-wrap: nowrap !important; overflow-x: auto !important; padding-bottom: 4px !important; }
          ...
          .ld-col-email,
          .ld-col-source,
          .ld-col-date { display: none !important; }
          .ld-card-meta { display: flex !important; }
        }
        .ld-card-meta { display: none; gap: 8px; align-items: center; flex-wrap: wrap; margin-top: 4px; }
      `}</style>
      ...
      <div className="ld-topbar" style={{
        ...
        <div>
          ...
          <p style={{ fontSize: "12px", color: "#94a3b8", marginTop: "2px" }}>Danh sách khách hàng tiềm năng</p>
        </div>
        <a href={`data:text/csv;charset=utf-8,${encodeURIComponent(csvContent)}`}
          ...
          <span className="ld-csv-text">Xuất CSV</span>
        </a>
      </div>

      <div className="ld-content" style={{ padding: "24px 28px" }}>

        {/* STATS */}
        <div className="ld-stats" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "14px", marginBottom: "20px" }}>
          {filters.map((f) => (
        ...
        {/* FILTERS */}
        <div className="ld-filters" style={{ display: "flex", gap: "8px", marginBottom: "16px" }}>
          {filters.map((f) => {
            const isActive = (!status && f.value === "all") || status === f.value
            return (
              <Link key={f.value} href={f.value === "all" ? "/admin/leads" : `/admin/leads?status=${f.value}`} style={{ textDecoration: "none", flexShrink: 0 }}>
                <div style={{
                  display: "flex", alignItems: "center", gap: "6px",
        ...
        <div style={{ background: "#fff", borderRadius: "12px", border: "0.5px solid #e2e8f0", overflow: "hidden" }}>
          ...
          {/* Header — ẩn mobile */}
          <div className="ld-table-header" style={{
            display: "grid",
          ...
          {!leads || leads.length === 0 ? (
            ...
            leads.map((lead: any, i: number) => {
              ...
              return (
                <div key={lead.id} className="ld-table-row" style={{
                  display: "grid",
                  gridTemplateColumns: "1fr 140px 130px 100px 120px 130px 120px",
                  padding: "14px 20px",
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {lead.message && (
                        <div style={{ fontSize: "11px", color: "#94a3b8", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "200px" }}>
                          {lead.message}
                        </div>
                      )}
                      {/* Meta chỉ hiện mobile */}
                      <div className="ld-card-meta">
                        <span style={{ fontSize: "12px", color: "#334155", fontWeight: 500 }}>{lead.phone || "—"}</span>
                        <span style={{ fontSize: "11px", fontWeight: 500, padding: "2px 8px", borderRadius: "20px", background: st.bg, color: st.color, border: `0.5px solid ${st.border}` }}>
                          {st.label}
                        </span>
                  ...
                  {/* SĐT — ẩn mobile */}
                  <div style={{ fontSize: "13px", color: "#334155" }}>{lead.phone || "—"}</div>

                  {/* Email — ẩn mobile */}
                  <div className="ld-col-email" style={{ fontSize: "12px", color: "#64748b", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                    {lead.email || "—"}
                  </div>

                  {/* Nguồn — ẩn mobile */}
                  <div className="ld-col-source" style={{ fontSize: "12px", color: "#64748b" }}>
                    {SOURCE_LABELS[lead.source] || lead.source}
                  </div>

                  {/* Trạng thái — ẩn mobile */}
                  <div className="ld-col-source">
                    <span style={{ fontSize: "11px", fontWeight: 500, padding: "3px 10px", borderRadius: "20px", background: st.bg, color: st.color, border: `0.5px solid ${st.border}` }}>
                      {st.label}
                    </span>
                  </div>

                  {/* Ngày tạo — ẩn mobile */}
                  <div className="ld-col-date" style={{ fontSize: "12px", color: "#64748b" }}>
                    {new Date(lead.created_at).toLocaleDateString("vi-VN")}
                  </div>
